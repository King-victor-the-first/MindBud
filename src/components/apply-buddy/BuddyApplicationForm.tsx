...
const applicationSchema = z.object({
  ...
  specializations: z.string().min(10, "Please list your specializations."),
...
export default function BuddyApplicationForm() {
  ...
  const handleSubmit = async (values: z.infer<typeof applicationSchema>) => {
    ...
    try {
      ...
      form.reset();
      // Potentially redirect the user or show a success message state
    } catch (error: any) {
      toast({
        variant: "destructive",
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
        ...
        <FormField control={form.control} name="licenseNumber" render={({ field }) => (
         ...
         <FormField control={form.control} name="specializations" render={({ field }) => (
            <FormItem>
                ...
                <FormControl><Textarea {...field} placeholder="e.g., Cognitive Behavioral Therapy (CBT), Trauma, Anxiety Disorders, etc."/></FormControl>
                <FormMessage />
            </FormItem>
        )} />
        ...
        <FormField control={form.control} name="reasonToJoin" render={({ field }) => (
            <FormItem>
                ...
                <FormControl><Textarea {...field} rows={5} placeholder="Tell us a little about your passion for mental health and why you'd like to volunteer."/></FormControl>
                <FormMessage />
            </FormItem>
        )} />
